...
const MenuFamily = ({ closeDrawer }: PropsPage) => {
  // -----------------------------------------------  HOOKS
  ...
  const menuItemsFamily = getMenuItemsFamily(t);
      ...
      <div className="relative  h-full! bg-(--bg-main)">
        ...
        >
          ...
          {menuItemsFamily.map((item) => {
            ...
            // Check if item has absolute path (starts with /)
            const isAbsolutePath = item.link.startsWith("/");
            const itemLink = isAbsolutePath
              ? item.link
