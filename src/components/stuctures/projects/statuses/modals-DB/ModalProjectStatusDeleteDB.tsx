...
import { StatusForIndexedDB, getStatusUID } from "../types";
...
export function ModalProjectStatusDeleteDB({
  open,
  close,
  selectedItems,
  cancelSelection,
  ...
  const { remove } = useIndexedDB<StatusForIndexedDB>({
    dbName: "deviceDB",
    storeName: "statuses",
  });
  ...
  const handleSubmitDelete = async () => {
    ...
    try {
      if (payment) {
        ...
        await Promise.all(
          selectedItems.map((item) => remove(getStatusUID(item) as number)),
        );
      }
      ...
      {() => (
        ...
        >
          ...
          <Flex direction="column" gap="3" ml="1">
            {selectedItems.map((item) => (
              <Flex key={getStatusUID(item)} align="center" gap="3">
